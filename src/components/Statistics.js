...
export const getStats = async (e, loggedIn, username, mode, setStats, onOpen) => {
  if(e)
    e.preventDefault();
  if(loggedIn) {
    await instance.get(`${process.env.REACT_APP_API_URL}/games/user/stats?username=${username}${(mode) ? `&mode=${mode}` : ``}`).then(async (response) => {
          ...
          {(props.stats.guessDistribution) && [...props.stats.guessDistribution, props.stats.numLosses].map((item, index) => 
            <Box key={`${mode}${index}`}>
              ...
              <Progress
                zIndex="-1"
                size='lg'
                colorScheme={(index===6) ? 'red' : (props.colorblindMode) ? 'blue' : 'green'}
                bgColor="gray.800"
